...
def main_worker(gpu, ngpus_per_node, args):
    ...
    args.normalizer = normalizer = datafree.utils.Normalizer(**registry.NORMALIZE_DICT['colored_fmnist'])
    ...
    if args.method=='deepinv':
        synthesizer = datafree.synthesis.DeepInvSyntheiszer_TinyImageNet(
                 ...
                 adv=args.adv, bn=args.bn, oh=args.oh, tv=args.tv, l2=args.l2, #tv=0.001, l2=0.0,
    ...
    counter = 0 
    x = torch.arange(10)
    targets = torch.squeeze(x.repeat(1, 8))
    for i, (input_img, target) in enumerate(train_loader):
        ...
        if i < 750:
            ...
            for ii in range(0,80):
                torch.save(vis_results['synthetic'][ii].cpu(), args.path_save_img+'image_'+str(ii + 80 * counter) + '.pth')
                torch.save(targets[ii].cpu(), args.path_save_targets+'label_'+str(ii + 80 * counter) + '.pth')
            # print(counter)
            counter += 1
            end = time.time()
