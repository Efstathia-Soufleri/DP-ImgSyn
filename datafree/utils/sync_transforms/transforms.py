...
__all__ = [ "Sync", "Multi","Compose", "ToTensor", "ToPILImage", "Normalize", "Resize", "Scale", "CenterCrop", "Pad",
...
class Sync(object):
    ...
    def __repr__(self):
        ...
        for transform in self.transforms:
            body += self._format_transform_repr(transform, "Transform: ")
        return '\n'.join(body)
...
class Multi(object):
    ...
    def __call__(self, *inputs):
        if len(self.transforms)==1:
            ...
            outputs = []
            for (input, trans) in zip( inputs, self.transforms ):
                outputs.append( trans(input) if trans is not None else input )
        return outputs
...
class Synthetic_Dataset(Dataset):
    def __init__(self, datapath, tgtpath, train_len = True, transform=None):
    ...
    def __len__(self):
        if self.train_len == 50000:
            return 50000 
        elif self.train_len == 60000:
            return 60000
        elif self.train_len == 10000:
            return 10000
        elif self.train_len == 5640:
            return 5640
        elif self.train_len == 162720:
            return 162720
        else :
            return 10000