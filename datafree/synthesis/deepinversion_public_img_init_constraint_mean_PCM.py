...
class DeepInvSyntheiszer_Public_Img_init_constraint_mean_PCM(BaseSynthesis):
    def __init__(self, teacher, student, num_classes, img_size, 
                 iterations=1000, lr_g=0.1, progressive_scale=False,
                 synthesis_batch_size=128, sample_batch_size=128, 
                 adv=0.0, bn=1, oh=1, tv=1e-5, l2=0.0, mcm = 0.1,
                 save_dir='run/deepinversion', transform=None,
                 normalizer=None, device='cpu',
                 # TODO: FP16 and distributed training 
                 autocast=None, use_fp16=False, distributed=False, privacy_constraint = 10.0):
        super(DeepInvSyntheiszer_Public_Img_init_constraint_mean_PCM, self).__init__(teacher, student)
    ...
    def synthesize(self, targets=None, images=None, mean=None):
        ...
        inputs_original.requires_grad_(False)
        dataset_mean = torch.Tensor(mean['mean']).to(inputs.device)
        ...
        for it in range(self.iterations):
            ...
            loss_privacy_constraint = torch.norm(inputs-inputs_original, 2)

            if mean is not None:
                # mean_constraint = torch.norm(inputs.mean((0,2,3)) - dataset_mean, 2)
                mean_constraint = torch.norm(inputs.mean((0,2,3)) - dataset_mean, 1)
            else:
                mean_constraint = 0


            loss = self.bn * loss_bn + self.oh * loss_oh + self.adv * loss_adv + self.tv * loss_tv + self.l2 * loss_l2 #+ loss_constraint * 100.0
            # print('total loss :', loss)
            # print('constraint loss :', loss_constraint)
            loss = loss + loss_privacy_constraint * self.privacy_constraint + self.mcm * mean_constraint
