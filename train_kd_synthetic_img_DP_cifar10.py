...
parser = argparse.ArgumentParser(description='PyTorch Synthetic Img Training')
# Basic
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    args.logger = datafree.utils.logger.get_logger(log_name, output='checkpoints/vanilla_kd_DP/log-%s-%s-%s-%s%s.txt'%(args.dataset, args.transfer_set, args.teacher, args.student, args.log_tag))
    if args.rank<=0:
    ...
    if os.path.isdir(args.transfer_set):
        ...
        print(f"Using {args.transfer_set} as train set")
    ...
    print('Accurcy of the pretraiend model: [Eval] Acc@1={acc1:.4f} Acc@5={acc5:.4f} Loss={loss:.4f}'.format(acc1=acc1, acc5=acc5, loss=val_loss))
        # return
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        _best_ckpt = 'checkpoints/vanilla_kd_DP/%s-%s-%s-%s.pth'%(args.dataset, args.transfer_set, args.teacher, args.student)
